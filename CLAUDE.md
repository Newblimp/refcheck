...
the same cross-product would assert nothing — and that no modifier is also in `EXCL`, where
it could never widen a term because the backward term walk stops there first. (`further` is
the one English entry in both, and has been dead vocabulary since before the split; the
test records it rather than silently tolerating it.)
...
Run with `npm test` (currently **762 tests**). Logic tests run under the fast `node`
