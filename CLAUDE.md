...
src/
  ...
  logic/                Pure, framework-free logic (unit-tested)
    ...
    cumulative.ts       canonicalCumulativeTerms — "eine erste Welle 10 … die Wellen
                        ...
                        a term introduced with a NUMBERING, and to what. Pure and
                        deliberately narrow; the list of cases it refuses is the
                        design (see Cumulative References below)
...
| `canonicalCumulativeTerms()`              | `logic/cumulative.ts`          | Which shortened terms are back-references to a numbered term of the SAME sign, and to what                                   |
| `isNumOrd()`                              | `logic/constants.ts`           | Is this word an ordinal **numbering** (`first`/`erste`) rather than a qualifier (`upper`/`obere`) — see the vocabulary table |
...
- The dominant term is the most frequent one, tie-broken by width and then by first
  appearance — but a term referred back to without its numbering never reaches that
...
#### The qualifier vocabulary (`EN_ORD` / `DE_ORD` in `constants.ts`)

"The ordinal pattern" is not only ordinals. `isOrd` matches a fixed list of 29 English and
42 German words — matched on the **raw lowercased word**, not the stem — and any of them
directly in front of the base noun widens the term to two words (`detectOrdStems` learns
the base noun from one such occurrence, and every later occurrence of that noun preceded by
one of these words is widened).

The list splits in two, and the split is what the cumulative rule turns on:

|             | **Numberings** — may be dropped on a later reference (see Cumulative References)                                                         | **Qualifiers** — widen the term, never dropped                                                                                                                                                                                                                                                                          |
| ----------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **EN** (29) | `first` `second` `third` `fourth` `fifth` `sixth` `seventh` `eighth` `ninth` `tenth`                                                     | `further` `other` `another` `next` `upper` `lower` `inner` `outer` `front` `rear` `left` `right` `top` `bottom` `primary` `secondary` `main` `auxiliary` `additional`                                                                                                                                                   |
| **DE** (42) | `erste` `ersten` `erstem` `erster` `erstes` · `zweite` `zweiten` `zweitem` `zweiter` `zweites` · `dritte` `dritten` · `vierte` `vierten` | `weitere` `weiteren` `weiterer` · `zusätzliche` `zusätzlichen` · `primäre` `primären` · `sekundäre` `sekundären` · `obere` `oberen` · `untere` `unteren` · `innere` `inneren` · `äußere` `äußeren` · `vordere` `vorderen` · `hintere` `hinteren` · `linke` `linken` · `rechte` `rechten` · `andere` `anderen` `anderer` |

`obere`/`upper` is one entry among nineteen and twenty-eight: **every** qualifier in the
right-hand column behaves the same way — "the auxiliary shaft 30" later written "the shaft
30" stays an inconsistency, because a lost qualifier is exactly the drafting slip this tool
exists to catch, and nothing in the text says which of the two the drafter meant.

**The numbering sets are not the same list.** `EN_NUM_ORD`/`DE_NUM_ORD` (`isNumOrd`) cover
every numbering the left-hand column holds **and reach further**: `eleventh`, `twelfth`,
and German `dritter/drittes/drittem`, `vierter/viertes/viertem` and all five endings of
`fünfte` through `zwölfte` — 12 EN and 65 DE forms in total. Those extra forms cannot widen
a term by themselves (they are not in `DE_ORD`, so the ordinal detector never sees them);
they matter only once the reference list or a manual override has already made the term
multi-word, and then the numbering is still droppable. So neither set contains the other,
and widening the ORD sets is a separate decision from widening the numbering sets — the
first changes every term in every document, the second only changes what may be folded.

`constants.test.ts` pins both halves of the table, so a word added to `EN_ORD`/`DE_ORD`
fails the suite until it has been classified as a numbering or a qualifier and this table
says which.

### Cumulative References (`logic/cumulative.ts`)

A term introduced with a **numbering** is routinely referred back to without it, because
the numbering only ever served to tell the siblings apart:
...
occurrence into the numbered term it refers back to, so none of the three is reported or
highlighted.
...
- **Exactly the numbering dropped** — the numbered term minus its first word, stem for
  stem (`erst well` → `well`). A term that lost anything else is a different term
- **A numbering, not any qualifier.** `isNumOrd` (`constants.ts`) matches `erste`/`first`,
  never `obere`/`upper` — and the same goes for every other qualifier the ordinal detector
  widens on (`inner`, `main`, `auxiliary`, `weitere`, `vordere`, …; the full list is
  tabulated under The Qualifier Vocabulary above). A drafter who wrote "the upper housing
  12" and later "the housing 12" may simply have lost the qualifier — that is the error,
  not a shorthand, so `EN_ORD`/`DE_ORD` are deliberately not reused here
- **One candidate.** A sign written as both "erste Welle 10" and "zweite Welle 10" is
  itself the inconsistency the tool exists to report; with two numbered forms there is no
  single term to fold into, so nothing is folded and the error stays visible
...
prints the numbered form however often the short one is written — the count tie-break must
...
of its own any more. A draft that never uses a numbering is unaffected.

The manual override still wins over everything: reducing the term takes the numbering off
every occurrence, so there is no numbered form left to fold into and the real term-to-sign
...
- [x] A **payload budget** runs in CI (`npm run budget`): critical path 41.5 KB / 50 KB,
      whole precached shell 60.0 KB / 70 KB
...
Run with `npm test` (currently **743 tests**). Logic tests run under the fast `node`
...
| `constants.test.ts`              | `likelySign`, `isClaimNumber` (terminators, indented, parens, mid-sentence, none, **Roman `I.` guard**, **CRLF**), `isSignToken` (prime/letter/range, **Roman + malformed rejection**), **`romanToInt`/`signVal`**, `compareSigns` (**Roman ordering, Arabic-before-Roman grouping**), article/ordinal helpers, **`isNumOrd` — that it accepts every German inflection and REJECTS the qualifiers `isOrd` accepts, which is the whole scope limit of the cumulative rule — plus a pin of the numbering/qualifier SPLIT of `EN_ORD`/`DE_ORD` (a word added there fails the suite until it is classified and CLAUDE.md's vocabulary table says which), and that neither set contains the other**, **`disKey`** — the declared source of truth for dismissal keys, which had no direct test at all while every dismissal test hard-coded the literals                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `extract.test.ts`                | sign/term consistency & inconsistencies, claims parentheses, claim-numbering (+ stable keys, CRLF), article errors (EN+DE), DE gender conflict, ordinal multi-word + `mwo` + `detectOrdStems` guards, bare terms, **prime signs**, **Roman step/substep signs + conflicts**, **ranges (to/bis/and/und/or/oder/through/dash/semicolon, EN+DE, with negatives, figure-word exclusion, `bis`/`Ansprüchen` never a term)**, **parenthesised sign groups**, **`noTermSigns`**, **bracketed paragraph numbers (`[0012]`) — now unit-tested directly**, **per-claim antecedent basis**, **claim dependency errors**, **`autoMW = false`** (previously never exercised by any call site), **multi-word terms from the reference list** (extension in both modes and in ranges, a different modifier left alone, bare occurrences reported as the whole phrase, DE, and both directions of the manual override — a reduction beating the list AND the ordinal detector), **cumulative references** (the numbered term kept as the sign's only term EN+DE, the whole passage reporting nothing, the shortened occurrence still counted and marked `cumulative`, order independence, no invented der/die/das conflict, antecedent basis in claims mode, the numbering taken from the reference list) **and the five cases that stay errors** (a dropped qualifier that is not a numbering, two numberings under one sign, a genuinely different term, a bare noun under a never-numbered sign, and a manual reduction winning with its conflicts intact)                                                                                                                                                                                                      |
| `claims.test.ts`                 | `segmentClaims` spans, `parseClaimRefs` (positions, offsets, lists, range expansion, DE, "preceding claims", trailing-comma negatives), `computeClaimGraph` (transitive ancestors, range/preceding ancestry, missing/forward/self typing, duplicate keys, acyclicity)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `cumulative.test.ts`             | The rule that suppresses the numbered-term noise, in isolation: what it folds (same sign, German inflections, a multi-word base, each sign against its own numbering, either document order) and — the half that matters — **what it refuses to fold** (across signs, a qualifier that is not a numbering, a sign with two numberings, a term that lost more than its numbering, a different noun, the wrong language, a raw/stem length disagreement). `cumKey` cannot collide whatever either half contains                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
