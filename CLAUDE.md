...
src/
  ...
  crt/crt.css           The CRT screen filter: the curved bezel, its glass edge
                        and refraction strips, the bowed raster, the roll and
                        power-on layers and their keyframes.
  ...
  crt/load.ts           The dynamic import in front of it (loadCrt/crtReady), the
                        same deferral LazyBee and LazyHelpDialog apply to a chunk
  ...
  hooks/
    ...
    useCrt.ts           The CRT screen filter: rsc_crt + <html data-crt>, exactly
                        like useTheme, except it waits for the lazily imported
                        stylesheet before setting the attribute the rules (and
                        the power-on animation) hang off
...
and whose corners squeeze in, an edge of thick glass with a bevel, a dispersion fringe
and a refraction that smears what you see through it, a raster of scanlines bowed to
match and drifting slowly down as if the scan
were a touch out of sync, a phosphor bloom on the text, a bright band rolling down over
it, a mains flicker and a power-on strike. Persisted under
...
- **The stylesheet is not on the critical path.** `styles.css` is inlined into
  `index.html`, so anything added there is downloaded by every first visit —
  including the ones that never switch this on. `src/crt/crt.css` is a dynamic
  import instead (`crt/load.ts`), which makes it its own asset: **1.0 KB gzipped,
  fetched on the first switch-on** (or on hover of the button, which preloads it the
  way the `?` button preloads the help dialog). The eager cost of the whole feature
  — icon, hook, button, two i18n strings — is **0.3 KB**, and `build/budget.ts` had
  to learn the chunk name or that win would be reported as a loss
...
**The edge of the glass** is four strokes on that same opening, painted in order: a wide
blurred white one (the bevel — the lit thickness, soft because it is shading rather than
a line), a warm stroke a hair OUTSIDE the opening and a cool one a hair INSIDE, and then
the crisp specular rim. The warm/cool pair is dispersion — thick glass splits what it
bends — and it is the cue that reads as _thick_ rather than as a rounded border. A
corner shade sits under them: glass is deepest where two curved edges meet, so the
darkening there is elliptical rather than rectangular, or the middle of each edge would
darken just as much.

**The refraction** is the one part of this filter that is not free, and it is four
`backdrop-filter: blur()` strips — one per edge — smearing the app's own pixels where the
glass is thickest. Four strips rather than one rim-shaped layer is what makes it
affordable: a backdrop-filter costs roughly its own area, and on a 100 KB description one
full-viewport ring took the editor's scroll from a **16.7 ms median frame to 65 ms**,
while these four (about a sixth of the viewport) cost **22 ms**. Below that there is a
floor — four small corner squares measured the _same_ as the strips — so this is as cheap
as a backdrop blur gets. Three things bound the cost, each pinned by a test: no filtered
layer may be full-bleed; all four sit **below** the drifting raster, since a moving layer
inside a backdrop would re-run the blur every frame rather than only when the app
repaints; and the whole thing is **off below the 860px breakpoint**, where the editor
fills the window and the smear would land on the drafter's own sentences instead of on
the chrome. The falloff is held high across the first half of each strip, because the
chrome under them sits in the MIDDLE of a strip rather than at its edge — a straight ramp
put the visible part of the smear almost entirely off-screen.

**It smears but does not displace**, and that is the requirement rather than a
compromise. Bending the pixels — a lens instead of a smear — is a dead end twice over.
It needs an SVG displacement map, and the only spatial source for one is `feImage`, which
does not load in Chromium at all: verified against an SVG data URI, a canvas-generated
PNG data URI and a served PNG, parsed from markup and created by script alike. Its failure
mode is not "no effect" but a uniform `scale/2` shift of the entire interface, since a
transparent map reads as a constant displacement. And even where it works, **hit testing
is not displaced with the pixels**, so every control under the band would answer a few
pixels from where it is drawn — which is the one thing a text editor cannot trade. A blur
moves nothing: the caret, the hover hit-testing and every button stay exactly where they
are painted.
...
| `rsc_crt`     | CRT screen filter: `'on'` or `'off'` (stored as the `data-crt` value itself)                                                                             |
...
- [x] **The CRT screen filter's stylesheet too** (`src/crt/load.ts`), by the same rule:
      ...
      on, everything except the refraction measures free: scrolling the editor holds 60fps
      (16.7 ms median frame, identical to filter-off) and keystroke-to-paint is unchanged
      within the noise between two filter-off runs. The refraction strips are the exception
      and were chosen with the numbers in hand — 22 ms scroll and +15-20 ms per keystroke,
      against 65 ms / 136 ms for the full-viewport version of the same effect (see CRT
      screen filter). Without them the scroll median under a 6× CPU throttle goes
      16.7 ms → ~18 ms, about 1 ms of which is the phosphor bloom
...
Run with `npm test` (currently **832 tests**). Logic tests run under the fast `node`
