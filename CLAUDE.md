...
src/
  ...
  crt/crt.css           The CRT screen filter: the curved bezel and its glass
                        edge, the bowed raster, the roll/power-on layers and
                        their keyframes.
...
and whose corners squeeze in, an edge of thick glass with a bevel and a dispersion
fringe, a raster of scanlines bowed to match and drifting slowly down as if the scan
...
**What is deliberately NOT there is a refraction blur.** Thick glass smears what you see
through it, and `backdrop-filter: blur()` on a rim-shaped layer does exactly that to the
app's own pixels. It was built, it looked right, and it came out again on the numbers: on
a 100 KB description it took scrolling the editor from a **16.7 ms median frame to 65 ms**
and keystroke-to-paint from **67 ms to 136 ms**. Four thin edge strips instead of one
full-viewport layer only recovered 24 ms / 97 ms, and four small corner squares measured
the _same as the strips_ — so the cost is not the filtered area, and there is no cheap
placement of it. Everything else in this filter measures free in the same harness, so the
glass is drawn rather than sampled.

Displacing the pixels for real — a lens instead of a smear — is a dead end twice over.
...
pixels from where it is drawn — which is the one thing a text editor cannot trade.
...
Run with `npm test` (currently **825 tests**). Logic tests run under the fast `node`
