...
| `isNumOrd()`                              | `logic/constants.ts`           | Is this word an ordinal **numbering** (`first`/`erste`) — a strict subset of `isOrd`                                        |
...
- **A numbering, not any qualifier.** `isNumOrd` (`constants.ts`) is a strict subset of
  `isOrd`: `erste`/`first`, never `obere`/`upper`. A drafter who wrote "the upper housing
...
Run with `npm test` (currently **741 tests**). Logic tests run under the fast `node`
