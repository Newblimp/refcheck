...
on an old cathode ray screen: green phosphor, scanlines, a vignetted tube, a bright band
rolling down the picture, a mains flicker and a power-on strike. Persisted under
...
- **It is a palette plus two overlays, never a `filter:` on a wrapper.** A filter (or
...
- **The chrome goes monospace via `--font-ui: var(--font-mono)`**, which is the one
  ...
  says so
- **`prefers-reduced-motion` keeps the look and drops the moving parts** (roll,
  flicker, power-on). Same stance the bee takes: what is suppressed is motion nobody
  asked for, not the thing that was asked for by name — a filter that switched on
  invisibly would just read as a broken button
...
- [x] **The CRT screen filter's stylesheet too** (`src/crt/load.ts`), by the same rule:
      ...
      under a 6× CPU throttle the median goes 16.7 ms → ~22 ms, and the **phosphor glow is
      not what costs it** — frame times with the editor's `text-shadow` forced off are the
      same within noise, so the overlays are the whole difference
...
Run with `npm test` (currently **817 tests**). Logic tests run under the fast `node`
