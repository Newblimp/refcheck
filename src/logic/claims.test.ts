...
const nums = (text: string): ClaimNumber[] => {
  const out: ClaimNumber[] = [];
  const re = /^[ \t]*(\d+)[.)]/gm;
  let m: RegExpExecArray | null;
  while ((m = re.exec(text)) !== null) {
    const written = m[1] ?? '';
    const s = m.index + m[0].indexOf(written);
    out.push({ value: parseInt(written, 10), start: s, end: s + written.length });
  }
  return out;
};
...
describe('computeClaimGraph', () => {
  ...
  it('builds transitive ancestor sets', () => {
    ...
    expect([...must(g.ancestors.get(1))]).toEqual([]);
    expect([...must(g.ancestors.get(2))]).toEqual([1]);
  ...
  it('bad references never create dependency edges (graph stays acyclic)', () => {
    ...
    const g = must(computeClaimGraph(text, nums(text)));
    expect([...must(g.ancestors.get(1))]).toEqual([]); // forward ref ignored as edge
    expect([...must(g.ancestors.get(2))]).toEqual([1]);
  });
});
