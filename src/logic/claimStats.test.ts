...
import type { ClaimNumber } from './claims.ts';
...
const statsFor = (text: string) => {
  ...
  for (const line of lines) {
    ...
    if (m) {
      const written = m[1] ?? '';
      const start = pos + line.indexOf(written);
      claimNums.push({ value: parseInt(written, 10), start, end: start + written.length });
    }
    pos += line.length + 1;
  }
  void res;
...
/** statsFor for the cases that go on to read fields off the result. */
const statsOf = (text: string) => must(statsFor(text), 'claim stats');

const claimSet = (...lines: string[]) => lines.join('\n');
...
describe('claimStats', () => {
  ...
  it('flags the EPO excess-claims threshold', () => {
    ...
    const s = statsOf(claimSet(...lines));
  ...
  it('does not flag a claim set at the DPMA threshold', () => {
    ...
    for (let n = 2; n <= THRESHOLDS.dpmaExcessClaims; n++)
      lines.push(`${n}. The device of claim 1, wherein feature ${n} applies.`);
    expect(statsOf(claimSet(...lines)).flags).not.toContain('dpmaExcessClaims');
  });
