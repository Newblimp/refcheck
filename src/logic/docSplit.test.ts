...
import type { SplitResult } from './docSplit.ts';
import type { Para } from './docx/read.ts';
...
const split = (body: string) => splitPatentDoc({ paragraphs: docxXmlToParagraphs(body) });
...
describe('refListWritable', () => {
  const H = (t: string) => para(t, { style: 'Heading1' });
  const check = (body: string) => refListWritable(split(body));
...
describe('sections never overlap', () => {
  const indicesOf = (paras: Para[]) => paras.map((p) => p.src.xmlStart);
  const disjoint = (r: SplitResult) => {
    const all = [
