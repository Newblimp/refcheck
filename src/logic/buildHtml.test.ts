...
describe('findAtPos', () => {
  it('locates the sign at a character position', () => {
    ...
    const at = must(findAtPos(13, r.signData, r.artErrors)); // inside "12"
    expect(at?.type).toBe('sign');
    expect(at?.type === 'sign' && at.sign).toBe('12');
  });

  it('locates an article error at its position', () => {
    const r = extractData('The housing 12 is large.', 'en'); // "The" → first-def
    const at = must(findAtPos(1, r.signData, r.artErrors)); // inside "The"
    expect(at?.type).toBe('art');
    expect(at?.type === 'art' && at.ae.article).toBe('the');
  });
  ...
  it('locates a bare term — the occurrence with no sign after it', () => {
    ...
    const at = must(
      findAtPos(text.lastIndexOf('banana') + 2, r.signData, r.artErrors, r.bareTerms)
    );
    expect(at?.type).toBe('bare');
    expect(at?.type === 'bare' && at.bt.term).toBe('banana');
    expect(at?.type === 'bare' ? at.bt.signs : null).toEqual(['10']);
  });

  it('does not report a bare term where the sign-attached occurrence is', () => {
    const text = 'A bendy banana 10 is next to another bendy banana.';
    const r = extractData(text, 'en');
    const at = must(findAtPos(text.indexOf('banana') + 2, r.signData, r.artErrors, r.bareTerms));
    expect(at?.type).toBe('sign');
  });
