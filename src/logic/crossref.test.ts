...
const desc = (txt: string) => extractData(txt, 'en', {}, true, false);
const claims = (txt: string) => extractData(txt, 'en', {}, true, true);
...
describe('computeCrossRef', () => {
  ...
  it('lists signs present in claims but missing from the description', () => {
    ...
    const x = must(computeCrossRef(d, c));
  ...
  it('detects the same sign mapped to different terms across buffers', () => {
    ...
    const conflict = must(x.signConflicts.find((s) => s.sign === '12'));
