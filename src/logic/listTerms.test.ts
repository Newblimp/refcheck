...
// noun last.
const toks = (phrase: string) => tokenize(phrase);
const extraFor = (idx: ListTermIndex, phrase: string, lang: Lang = 'en') => {
  const tt = toks(phrase);
  return listExtra(idx, tt, stem(must(tt[tt.length - 1]).word, lang), lang);
};
...
describe('listTermIndex', () => {
  it('indexes multi-word entries and skips single-word ones', () => {
    ...
    expect(must(idx.byBase.get(stem('unit', 'en'))).get(stem('control', 'en'))).toHaveLength(1);
