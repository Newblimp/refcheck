...
import type { Para, ParaSrc } from './read.ts';

const NUMPR = '<w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr>';

/** A paragraph as the reader would hand it over. */
/** A partial paragraph override — `src` may be given partially too. */
type ParaOver = Partial<Omit<Para, 'src'>> & { src?: Partial<ParaSrc> };
type ParaBase = Omit<ParaOver, 'src'>;

const p = (over: ParaOver = {}): Para => ({
  text: '',
  style: '',
  numbered: false,
  numId: null,
...
const listItem = (over: ParaOver = {}): Para =>
  p({
    numbered: true,
    numId: 1,
    ...over,
