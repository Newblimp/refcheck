...
const texts = (xml: string) => docxXmlToParagraphs(xml).map((p) => p.text);
...
describe('docxXmlToParagraphs', () => {
  it('joins runs inside a paragraph with NO separator', () => {
    ...
    const xml =
      '<w:p><w:r><w:t>hous</w:t></w:r><w:r><w:t>ing</w:t></w:r>' +
...
describe('readDocx', () => {
  ...
  it('rejects a non-zip (a legacy binary .doc) with code notZip', () => {
    ...
    try {
      ...
      expect(e instanceof DocxError && e.code).toBe('notZip');
    }
  });
  it('rejects a zip without word/document.xml with code noDocument', () => {
    ...
    try {
      ...
      expect(e instanceof DocxError && e.code).toBe('noDocument');
    }
  });
});
