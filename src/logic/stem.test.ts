...
describe('stem dispatch', () => {
  ...
  it('defaults to the English stemmer for an unknown language', () => {
    // 'fr' is not a supported language; the dispatcher falls back to English,
    // which is the behaviour being pinned here.
    expect(stem('housings', 'fr' as Lang)).toBe(stemEn('housings'));
  });
});
